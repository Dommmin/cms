...
When a page is published, unpublished, or processed by the scheduled publishing task, the system can notify configured webhooks so external storefront, CDN, or search indexing integrations can refresh their cached content. For the bundled storefront, create an active webhook for `page.published` and `page.unpublished` that points to `/api/cms/revalidate`, uses the same secret as the storefront environment, and is exposed as a reachable HTTPS endpoint outside local development. The platform now applies one shared delivery standard to these outbound webhooks, so failed attempts, timeouts, and signature headers are tracked consistently.
...
- **Cards / Canvas views** — use **Cards** for the full structured editor, or **Canvas** to see visual previews of all Page Builder block types directly in the page flow. In Canvas, click a block to select it, double-click it or use **Edit** to open the inspector, and edit simple text such as titles, subtitles, descriptions, and CTA labels directly on the preview.
- **Simple / Advanced modes** — use **Simple** for Canvas-first editing with the most common content fields, or **Advanced** when you need all block options, linked content, animation, custom CSS, element IDs, or lock controls. Switching modes does not remove existing settings.
...
- **Image editing** — selected images can be edited from the media picker. Use aspect presets, zoom, rotation, and click the image to set a focal point. Cropped variants are saved as separate media items and can be selected in rich text image metadata when available.
...
The mobile MVP also exposes customer account basics: login, registration, profile edits, order history, wishlists, product reviews, cart discounts and checkout redirects.
