...
final class CustomReportController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'data_source' => ['required', 'string', 'in:'.implode(',', $this->builder->getAvailableDataSources())],
            'metrics' => ['required', 'array'],
            'metrics.*' => ['string', 'in:'.implode(',', $this->builder->getAvailableMetrics())],
            'dimensions' => ['nullable', 'array'],
            'filters' => ['nullable', 'array'],
            'filters.*.field' => ['sometimes', 'string', 'max:64', 'regex:/^[a-zA-Z_][a-zA-Z0-9_]*$/'],
            'filters.*.operator' => ['sometimes', 'string', 'in:=,!=,>,<,>=,<=,like,in,not_in'],
            'group_by' => ['nullable', 'array'],
            'group_by.*.field' => ['sometimes', 'string', 'max:64', 'regex:/^[a-zA-Z_][a-zA-Z0-9_]*$/'],
            'group_by.*.type' => ['sometimes', 'string', 'in:date,month,week,plain'],
