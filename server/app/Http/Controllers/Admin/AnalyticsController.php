...
class AnalyticsController extends Controller
{
    ...
    public function jpkExport(Request $request): HttpResponse
    {
        ...
        $month = $request->filled('month')
            ? Date::parse($request->input('month'))->startOfMonth()
    ...
    {
        /** @var Carbon $from */
        $from = $request->filled('from') ? Date::parse($request->input('from'))->startOfDay() : Date::now()->subDays(29)->startOfDay();
        /** @var Carbon $to */
        $to = $request->filled('to') ? Date::parse($request->input('to'))->endOfDay() : Date::now()->endOfDay();
