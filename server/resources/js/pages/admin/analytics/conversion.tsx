            ...
            <Wrapper>
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                    <TabsList>
                        ...
                        <TabsTrigger value="promo">Promo Attribution</TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="landing" className="space-y-6">
                        <div className="rounded-xl border bg-card">
                            <div className="border-b px-6 py-4">
                                <h2 className="text-base font-semibold">Landing Pages with Conversions</h2>
                                <p className="text-xs text-muted-foreground">
                                    Analyze which landing pages generate sessions, checkouts, and sales.
                                </p>
                            </div>
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody>
                                            {data.landing_pages.map((row) => {
                                                const cr = row.sessions > 0 ? round((row.conversions / row.sessions) * 100, 1) : 0;
                                                return (
                                                    <tr key={row.url} className="border-b last:border-0 hover:bg-muted/30">
                                                        <td className="px-6 py-3 font-mono text-xs">{row.url}</td>
                                                        <td className="px-6 py-3 text-right tabular-nums">{row.sessions}</td>
                                                        <td className="px-6 py-3 text-right tabular-nums">{row.conversions}</td>
                                                        <td className="px-6 py-3 text-right tabular-nums">{cr}%</td>
                                                        <td className="px-6 py-3 text-right font-medium text-green-600 dark:text-green-400 tabular-nums">
                    ...
                    <TabsContent value="promo" className="space-y-6">
                        <div className="rounded-xl border bg-card">
                            <div className="border-b px-6 py-4">
                                <h2 className="text-base font-semibold">Promotion Code Attribution</h2>
                                <p className="text-xs text-muted-foreground">
                                    Track conversion metrics and revenue generated by promotional coupon codes.
                                </p>
                            </div>
                            {(!data.promotions || data.promotions.length === 0) ? (
                                <div className="p-10 text-center text-sm text-muted-foreground">
                                    No promotional sales recorded in this period.
                                </div>
                            ) : (
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody>
                                            {data.promotions.map((row) => (
                                                <tr key={row.code} className="border-b last:border-0 hover:bg-muted/30">
                                                    ...
                                                    <td className="px-6 py-3 text-right tabular-nums font-medium">{row.purchases}</td>
