...
function HeroBannerPreview({ cfg }: { cfg: Cfg }) {
    return (
        <div className="relative flex min-h-[420px] items-center justify-center bg-gradient-to-br from-slate-800 to-slate-600 px-8 py-16 text-white"
            style={{ minHeight: cfg.min_height ? `${cfg.min_height}px` : undefined }}>
            <div className="relative z-10" style={{ textAlign: ((cfg.text_alignment as string) || 'center') as React.CSSProperties['textAlign'] }}>
                {cfg.title && <h1 className="mb-4 text-4xl font-bold leading-tight">{cfg.title as string}</h1>}
                {cfg.subtitle && <p className="mb-8 text-xl opacity-90 max-w-2xl">{cfg.subtitle as string}</p>}
                {cfg.cta_text && (
                    <a href={cfg.cta_url as string || '#'}
...
function RichTextPreview({ cfg }: { cfg: Cfg }) {
    const widthClass = cfg.max_width === 'narrow' ? 'max-w-xl' : cfg.max_width === 'wide' ? 'max-w-5xl' : cfg.max_width === 'full' ? 'max-w-none' : 'max-w-3xl';
    return (
        <div className={`mx-auto px-8 py-10 ${widthClass}`}>
            {cfg.content ? (
                ...
                <p className="italic text-muted-foreground">Rich text — no content yet.</p>
            )}
        </div>
    );
}
...
function TwoColumnsPreview({ cfg }: { cfg: Cfg }) {
    ...
    const gridClass = ratioMap[(cfg.ratio as string) || '50-50'] || 'grid-cols-2';

    return (
        <div className={`grid gap-8 px-8 py-10 ${gridClass}`}>
            <div className="prose prose-slate max-w-none">
                {cfg.left_content
                    ? <div dangerouslySetInnerHTML={{ __html: cfg.left_content as string }} />
                    : <div className="h-24 rounded-lg bg-muted" />}
            </div>
            <div className="prose prose-slate max-w-none">
                {cfg.right_content
                    ? <div dangerouslySetInnerHTML={{ __html: cfg.right_content as string }} />
                    : <div className="h-24 rounded-lg bg-muted" />}
            </div>
        </div>
    );
        ...
        <div className="grid grid-cols-3 gap-6 px-8 py-10">
            {cols.map((col, i) => (
                <div key={i} className="space-y-3">
                    {col.title && <h3 className="font-semibold">{col.title as string}</h3>}
                    {col.content
...
function AccordionPreview({ cfg }: { cfg: Cfg }) {
    const items = (cfg.items as Array<{ title?: string; content?: string }> | undefined) ?? [];
    return (
        <div className="mx-auto max-w-3xl px-8 py-10">
            {cfg.title && <h2 className="mb-6 text-2xl font-bold">{cfg.title as string}</h2>}
            <div className="divide-y rounded-lg border">
                {items.length > 0 ? items.map((item, i) => (
                    ...
                    <div className="p-6 text-center text-sm text-muted-foreground">No items added yet</div>
                )}
            </div>
        </div>
    );
...
function TabsPreview({ cfg }: { cfg: Cfg }) {
    const tabs = (cfg.tabs as Array<{ title?: string; content?: string }> | undefined) ?? [];
    return (
        <div className="px-8 py-10">
            ...
            <div className="flex gap-1 border-b mb-6">
                {tabs.map((tab, i) => (
                    <div key={i}
                        className={`px-4 py-2 text-sm font-medium cursor-pointer ${i === 0 ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground'}`}>
                        {tab.title || `Tab ${i + 1}`}
                    </div>
                ))}
                {tabs.length === 0 && (
                    <div className="px-4 py-2 text-sm text-muted-foreground">No tabs added yet</div>
                )}
            </div>
            {/* First tab content */}
            {tabs[0]?.content ? (
...
function TestimonialsPreview({ cfg }: { cfg: Cfg }) {
    const items = (cfg.items as Array<{ author?: string; role?: string; content?: string; rating?: number }> | undefined) ?? [];
    const showRating = cfg.show_rating !== false;

    return (
        <div className="px-8 py-12">
            {cfg.title && <h2 className="mb-8 text-center text-2xl font-bold">{cfg.title as string}</h2>}
            {items.length > 0 ? (
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {items.map((item, i) => (
                        <div key={i} className="rounded-xl border bg-card p-6 shadow-sm">
                            ...
                            {item.content && (
                                <p className="mb-4 text-sm text-muted-foreground">"{item.content}"</p>
                            )}
                            <div>
            ...
            <div className={`grid gap-4`} style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
                {Array.from({ length: Math.min(count, cols) }).map((_, i) => (
                    <div key={i} className="rounded-lg border bg-card p-4">
                        ...
                        {cfg.show_price !== false && <div className="h-3 w-1/3 rounded bg-muted" />}
                    </div>
                ))}
            </div>
        </div>
...
function PromotionalBannerPreview({ cfg }: { cfg: Cfg }) {
    ...
    return (
        <div className="px-8 py-14 text-center" style={{ backgroundColor: bg, color: fg }}>
            {cfg.title && <h2 className="mb-2 text-3xl font-bold">{cfg.title as string}</h2>}
            {cfg.subtitle && <p className="mb-2 text-lg opacity-90">{cfg.subtitle as string}</p>}
            {cfg.badge_text && (
...
function NewsletterSignupPreview({ cfg }: { cfg: Cfg }) {
    ...
    return (
        <div className="px-8 py-12 text-center" style={{ backgroundColor: bg }}>
            {cfg.title && <h2 className="mb-2 text-2xl font-bold">{cfg.title as string}</h2>}
            {typeof cfg.description === 'string' && cfg.description && <p className="mb-6 text-muted-foreground">{cfg.description}</p>}
...
function VideoEmbedPreview({ cfg }: { cfg: Cfg }) {
    ...
    const aspectClass = ratioMap[(cfg.aspect_ratio as string) || '16:9'] || 'aspect-video';

    return (
        <div className="px-8 py-10">
            {cfg.title && <h2 className="mb-4 text-xl font-semibold">{cfg.title as string}</h2>}
            <div className={`relative ${aspectClass} rounded-lg border bg-slate-900 overflow-hidden flex items-center justify-center`}>
                {cfg.video_url ? (
                    <iframe
                    ...
                    <div className="flex flex-col items-center gap-2 text-white/60">
                        <div className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-white/30">
                            <div className="ml-1 h-0 w-0 border-b-[10px] border-t-[10px] border-l-[18px] border-b-transparent border-t-transparent border-l-white/60" />
                        </div>
...
function GenericBlockPreview({ type }: { type: string }) {
    return (
        <div className="mx-8 my-4 rounded-lg border border-dashed bg-muted/20 p-6 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Preview not available for this block type</p>
        </div>
    );
}
...
function PageBlockPreview({ block }: { block: PreviewBlock }) {
    ...
    switch (block.type) {
        case 'hero_banner':          return <HeroBannerPreview cfg={c} />;
        case 'rich_text':            return <RichTextPreview cfg={c} />;
        case 'two_columns':          return <TwoColumnsPreview cfg={c} />;
        case 'three_columns':        return <ThreeColumnsPreview cfg={c} />;
        case 'accordion':            return <AccordionPreview cfg={c} />;
        case 'tabs':                 return <TabsPreview cfg={c} />;
        case 'testimonials':         return <TestimonialsPreview cfg={c} />;
        case 'featured_products':    return <FeaturedProductsPreview cfg={c} />;
        case 'categories_grid':      return <CategoriesGridPreview cfg={c} />;
        case 'promotional_banner':   return <PromotionalBannerPreview cfg={c} />;
        case 'newsletter_signup':    return <NewsletterSignupPreview cfg={c} />;
        case 'video_embed':          return <VideoEmbedPreview cfg={c} />;
        case 'image_gallery':        return <ImageGalleryPreview cfg={c} />;
        case 'custom_html':          return <CustomHtmlPreview cfg={c} />;
        case 'form_embed':           return <FormEmbedPreview cfg={c} />;
        default:                     return <GenericBlockPreview type={block.type} />;
    }
}
        ...
        <>
            ...
            <div className="min-h-screen bg-background font-sans">
                ...
                <div className="sticky top-0 z-50 border-b bg-amber-50 px-6 py-2 text-center text-xs text-amber-800 dark:bg-amber-950 dark:text-amber-200">
                    ...
                    {' — '}
                    {pageTitle}
                    <span className="ml-1 text-amber-600 dark:text-amber-400">/{page.slug}</span>
                </div>
                ...
                {sections.length === 0 ? (
                    <div className="flex min-h-[60vh] items-center justify-center">
                        <div className="text-center text-muted-foreground">
                            ...
                            <p className="text-sm">Add sections and blocks in the builder, then save.</p>
                        </div>
                    </div>
                ) : (
