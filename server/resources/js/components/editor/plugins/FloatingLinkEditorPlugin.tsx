...
function FloatingLinkEditor({
    ...
    const $updateLinkEditor = useCallback(() => {
        ...
        if ($isRangeSelection(selection)) {
            ...
            if (isLinkEditMode) setEditedLinkUrl(linkParent ? linkParent.getURL() : $isLinkNode(node) ? node.getURL() : 'https://');
        }
        ...
        if (
            ...
            if (rootElement !== null) positionEditorElement(editorElem, null, anchorElem);
    ...
    return (
        <div ref={editorRef} className="link-editor absolute z-50 flex items-center gap-1.5 rounded-lg border bg-popover px-2 py-1.5 shadow-md">
            {isLink && (isLinkEditMode ? (
                <>
                    <input
                        ref={inputRef}
                        className="link-input flex-1 min-w-[200px] bg-transparent text-sm outline-none"
                        value={editedLinkUrl}
                    ...
                    <button
                        ...
                        onClick={() => { setEditedLinkUrl('https://'); setIsLinkEditMode(false); }}
