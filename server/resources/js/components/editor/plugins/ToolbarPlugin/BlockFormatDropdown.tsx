...
const BLOCK_TYPES: { type: BlockType; label: string; icon: React.ReactNode }[] = [
    { type: 'paragraph', label: 'Normal', icon: <Type className="h-4 w-4" /> },
    ...
    { type: 'h4', label: 'Heading 4', icon: <Heading4 className="h-4 w-4" /> },
    { type: 'h5', label: 'Heading 5', icon: <Heading5 className="h-4 w-4" /> },
    ...
    { type: 'check', label: 'Check List', icon: <ListChecks className="h-4 w-4" /> },
...
function formatHeading(editor: LexicalEditor, type: 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6'): void {
    editor.update(() => {
        const selection = $getSelection();
        if ($isRangeSelection(selection)) $setBlocksType(selection, () => $createHeadingNode(type));
    });
}
...
export default function BlockFormatDropdown({ blockType, disabled }: Props): JSX.Element {
    const [editor] = useLexicalComposerContext();
    const currentBlock = BLOCK_TYPES.find((b) => b.type === blockType) ?? BLOCK_TYPES[0];

    const handleSelect = (type: BlockType) => {
        switch (type) {
            case 'paragraph': formatParagraph(editor); break;
            case 'h1': formatHeading(editor, 'h1'); break;
            case 'h2': formatHeading(editor, 'h2'); break;
            case 'h3': formatHeading(editor, 'h3'); break;
            case 'h4': formatHeading(editor, 'h4'); break;
            case 'h5': formatHeading(editor, 'h5'); break;
            case 'bullet': formatBulletList(editor, blockType); break;
            case 'number': formatNumberedList(editor, blockType); break;
            case 'check': formatCheckList(editor, blockType); break;
            case 'quote': formatQuote(editor); break;
            case 'code': formatCode(editor); break;
        }
    };
    ...
    return (
        <DropDown
            disabled={disabled}
            label={
                <span className="flex items-center gap-1.5 text-sm min-w-[120px]">
                    {currentBlock.icon}
                    {currentBlock.label}
                </span>
            }
