...
- **Page Builder** — drag-and-drop blocks, sections, reusable blocks, draft/publish versioning, live preview, mobile preview, undo/redo, copy/paste blocks
  ...
  - **B10 Design System Tokens** — Theme model: `tokens`, `dark_tokens`, `typography`, `font_sources`, `spacing`, `buttons`, `containers`, `branding`; `globals.css` token contract + consolidated `--store-shell-width`/`--store-card-radius` aliases; `HandleAppearance` + `ThemeStyles` whitelist/sanitize inject `:root` and `.dark`; storefront composition primitives in `client/components/composition/`; blocks use token-driven colors (no hardcoded `text-white`/gray overlays)
