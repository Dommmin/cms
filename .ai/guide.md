...
- **Page Builder** — drag-and-drop blocks, sections, reusable blocks, draft/publish versioning, live preview, mobile preview, undo/redo, copy/paste blocks
  ...
  - **Block contract** — every `PageBlockTypeEnum` value must have a `config('blocks.block_types')` registry entry with `schema` + `data_strategy`; storefront `blockRegistry` must register every enum value (`client/tests/unit/block-registry.test.ts`)
