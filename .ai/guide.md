...
- **Page Builder** — drag-and-drop blocks, sections, reusable blocks, draft/publish versioning, live preview, mobile preview, undo/redo, copy/paste blocks
  ...
  - **Block contract** — every `PageBlockTypeEnum` value must have a `config('blocks.block_types')` registry entry with `schema` + `data_strategy`; storefront `blockRegistry` must register every enum value (`client/tests/unit/block-registry.test.ts`); `BlockValidationService` validates schema/registry/data_strategy/context_dependencies/allowed_children at admin save (`PageBuilderSnapshotValidator`, reusable blocks) and storefront pre-render (`usePreRenderBlockValidation` in `block-renderer.tsx`); blocks with `data_strategy: server` must resolve relations in `PageResource` (no client `fetch` / `useEffect` network in renderers); `hybrid` blocks submit via `client/components/page-builder/mutations/` only; `context_dependencies` for `featured_products` are resolved server-side in `PageResource` + `PageRenderContext`
