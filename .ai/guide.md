...
- **Page Builder** — drag-and-drop blocks, sections, reusable blocks, draft/publish versioning, live preview, mobile preview, undo/redo, copy/paste blocks
  ...
  - **B17 Storefront Renderer Tests** — `client/` has Vitest (`npm run test:ui`) with `tests/unit/page-builder-renderers.test.tsx`; the smoke suite renders all 30 storefront Page Builder block components with minimal fixtures and covers `SectionRenderer` plus `SectionLazyWrapper`, with one test case per block type for easy failure isolation
