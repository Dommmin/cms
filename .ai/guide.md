...
- **Page Builder** — drag-and-drop blocks, sections, reusable blocks, draft/publish versioning, live preview, mobile preview, undo/redo, copy/paste blocks
  ...
  - **B11 Inline Editing (Canvas View)** — Page builder has `viewMode` toggle (Cards/Canvas); `CanvasView` renders real admin previews for `hero_banner`, `rich_text`, `call_to_action`, `image_gallery`, and `featured_products`, with section backgrounds/layout/padding; blocks are selectable with hover outlines and Edit buttons; double-click opens sidebar inspector; simple text fields (`title`, `heading`, `subtitle`, `description`, `primary_label`, `secondary_label`) can be edited inline and update block configuration; keyboard shortcuts: Ctrl+S (save), Delete (remove), Ctrl+D (duplicate), Esc (deselect), / (quick add); `addQuickBlock()` auto-creates section if needed
  - **B12 Image Editing** — `ImageEditorModal` (react-image-crop) with aspect ratio presets, rotation, focal point picker; integrated into `MediaPickerModal` with Edit button on images; backend `MediaController::crop()` endpoint generates cropped variants via Intervention Image
