...
- **Page Builder** — drag-and-drop blocks, sections, reusable blocks, draft/publish versioning, live preview, mobile preview, undo/redo, copy/paste blocks
  ...
  - **B11 Inline Editing (Canvas View)** — Page builder has `viewMode` toggle (Cards/Canvas); `CanvasView` component renders sections with visual backgrounds and block placeholders; blocks are selectable with hover outlines and Edit buttons; double-click opens sidebar inspector; keyboard shortcuts: Ctrl+S (save), Delete (remove), Ctrl+D (duplicate), Esc (deselect), / (quick add); `addQuickBlock()` auto-creates section if needed
