...
- **Page Builder** — drag-and-drop blocks, sections, reusable blocks, draft/publish versioning, live preview, mobile preview, undo/redo, copy/paste blocks
  ...
  - **B10 Design System Tokens** — Theme model: `tokens`, `dark_tokens`, `typography`, `font_sources`, `spacing`, `buttons`, `containers`, `branding`; `globals.css` token contract + consolidated `--store-shell-width`/`--store-card-radius` aliases; `HandleAppearance` + `ThemeStyles` whitelist/sanitize inject `:root` and `.dark`; storefront layout primitives in `client/components/composition/` (`index.ts` barrel, shared class maps in `styles.ts`, `Container`/`Section`/`Grid`/`Stack`/`Surface`/`PageHeader`/`BlockHeader`/`CTASection`/`EmptyState`/`AnimateOnView`); shadcn `Alert` in `client/components/ui/alert.tsx` (semantic variants: info/success/warning/destructive); page builder blocks with section titles use `BlockHeader`; page builder (`section-renderer`, column blocks, CTA) and blog import from composition — no duplicated variant/padding/container maps; blocks use token-driven colors (no hardcoded `text-white`/gray overlays)
  ...
  - **BlockRegistry** — `block-renderer.tsx` is registry-only (`blockRegistry` lookup); missing entry logs a warning and throws; `BlockErrorBoundary` in `section-renderer.tsx` and `slot-renderer.tsx` shows per-block fallback UI
  ...
  - **Block schema export** — `php artisan blocks:export` writes `storage/app/blocks.schema.json`; committed snapshot at `tests/Unit/PageBuilder/snapshots/blocks.schema.json`; CI job `blocks-contract` runs export → `diff` snapshot → `php artisan blocks:export --check` → generated TS checks (server + client) → Pest contract tests → Vitest storefront registry; mirrored in `make check` and `scripts/check-blocks-contract.sh`
  - **Block TS types (generated, headless)** — codegen lives in `server/scripts/` (contract owner); **admin** (`docker compose exec php npm run generate:blocks-types`) → `server/resources/js/types/generated/blocks.generated.ts`; **storefront** (`docker compose exec node npm run generate:blocks-types`) → `client/types/generated/blocks.generated.ts` (reads snapshot + shared codegen lib via read-only Docker mounts — no runtime coupling); do not edit generated files manually
  - **Block contract** — every `PageBlockTypeEnum` value must have a `config('blocks.block_types')` registry entry with `schema` + `data_strategy`; storefront `blockRegistry` must register every enum value (`client/tests/unit/block-registry.test.ts`); `BlockValidationService` validates at admin save (`PageBuilderSnapshotValidator`) and storefront pre-render; optional `business_rules` enforced by `PageBuilderRulesService` (keys = `block_type`); AI composition via `PageBuilderAiComposerService` (`page_versions.source = ai`); `page_blocks.schema_version` defaults to 1 on sync
  - **`featured_products` server data** — `data_strategy: server`; `PageResource` resolves `filter_mode=featured` products server-side (same product payload shape as manual relations) and injects them under `relations[].relation_key=products`; storefront block reads relations only (no client fetch); empty state renders skeleton placeholders (`client/tests/unit/featured-products-block.test.tsx`, `server/tests/Feature/Api/PageFeaturedProductsBlockTest.php`)
- **Themes** — multiple themes with full design system (colors, `dark_tokens`, typography, `font_sources`, spacing, buttons, containers, `branding`, `draft_tokens`); default storefront preset `default` (Blog-quality indigo + Space Grotesk/Inter); activate/deactivate; per-page override via `theme_id` exposed in `PageResource` and applied in `cms-dynamic-page` via `ThemeStyles`; CSS variables injected via `HandleAppearance` (admin, incl. `.dark`) and `ThemeStyles` + `ThemeFontLoader` (storefront); theme editor with live preview panel; logo/favicon from `theme.branding`
...
- **Catalog/content demo seed pack** — `DefaultBlogSeeder`, `AttributeDefinitionSeeder`, `CategoryAttributeSchemaSeeder`, `DemoProductSeeder`, `DemoProductVariantSeeder`, `DemoBlogSeeder`, `DemoCmsPageSeeder`, `DemoMetafieldSeeder`, `PageBuilderShowcaseSeeder`, `ThemeShowcaseSeeder`, and `DesignSystemShowcaseSeeder` now build an offline-ready final verification dataset with default blog, category-owned schemas, product-level core attributes, legacy-safe variant options, public/private metafields, seeded storefront system pages, a `/page-builder-showcase` page with all 30 block types (grouped into Marketing / Content / Ecommerce / Forms / Media / Layout sections), a `/theme-showcase` page for design-token QA (typography / buttons / cards / surfaces / spacing / colors), a `/design-system-showcase` page for composition + UI primitive QA (15 groups — see `docs/design-system-showcase.md`), and local placeholder assets
