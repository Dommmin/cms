...
export default function OrderDetailPage() {
  ...
  return (
    <div className="space-y-4">
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            ...
            {order.shipment.tracking_number && (
              <>
                ...
                <dd className="font-mono font-medium">{order.shipment.tracking_number}</dd>
              </>
            )}
            <dt className="text-muted-foreground">{t('order.status', 'Status')}</dt>
            <dd className="font-medium capitalize">{order.shipment.status}</dd>
          </dl>
        </div>
      )}
                ...
                <div>
                  ...
                  {item.variant_sku && (
                    <p className="text-muted-foreground text-xs">
                      {t('order.sku', 'SKU')}: {item.variant_sku}
                    </p>
                  )}
                  <p className="text-muted-foreground text-xs">
      ...
      {order.payment && (
        <div className="border-border bg-card rounded-xl border p-4">
          <h3 className="mb-2 text-sm font-semibold">{t('order.payment', 'Payment')}</h3>
      ...
      {['delivered', 'shipped'].includes(order.status) && !returnSuccess && (
        <div>
          ...
          >
            {showReturnForm
              ? t('order.hide_return_form', 'Hide Return Form')
              : t('order.show_return_form', 'Request Return / Complaint')}
          </button>
        </div>
      )}
      ...
      {/* Return form */}
      {showReturnForm && (
        <form onSubmit={handleReturnSubmit} className="border-border bg-card rounded-xl border p-4">
          <h3 className="mb-4 font-semibold">
            {t('order.return_request', 'Return / Complaint Request')}
          </h3>
