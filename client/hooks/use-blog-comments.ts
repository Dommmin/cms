...
export function useBlogVote(slug: string) {
  ...
  return useMutation({
    mutationFn: (vote: 'up' | 'down') =>
      apiPost<{ votes_up: number; votes_down: number; user_vote: 'up' | 'down' | null }>(
    ...
    onSuccess: (result) => {
      ...
      queryClient.setQueryData(
        ['blog-post', slug],
        (old: { votes_up?: number; votes_down?: number; user_vote?: string | null } | undefined) =>
          old
            ? {
                ...old,
...
export function useRecordBlogView(slug: string) {
  return useMutation({
    mutationFn: () => apiPost<{ views_count: number }>(`/blog/posts/${slug}/view`),
  });
}
