...
function CommentForm({ slug, parentId, replyingTo, onCancel }: CommentFormProps) {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      {replyingTo && (
        <p className="text-muted-foreground flex items-center gap-1.5 text-xs">
          ...
          <span className="text-primary font-semibold">{replyingTo}</span>
        </p>
      )}
      <textarea
        value={body}
    ...
    <div className="flex gap-3 sm:gap-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        {user && (
          ...
          >
            <CornerDownRight className="h-3 w-3" aria-hidden="true" />
            {showReply ? t('common.cancel', 'Cancel') : t('blog.reply', 'Reply')}
          </button>
        )}
    ...
    <section className="mt-12 border-t pt-8">
      <h2 className="mb-8 flex items-center gap-2 text-xl font-bold">
        ...
        {data?.meta.total != null && (
          <span className="text-muted-foreground text-base font-normal">({data.meta.total})</span>
        )}
      </h2>
      ...
      {user ? (
        ...
        <div className="border-border bg-muted/30 mb-10 rounded-lg border p-4 text-sm">
          <Link href={lp('/login')} className="text-primary font-semibold hover:underline">
            {t('blog.login_to_comment', 'Log in to leave a comment')}
          </Link>
        </div>
      )}
