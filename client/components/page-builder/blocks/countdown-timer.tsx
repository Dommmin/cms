...
export function CountdownTimerBlock({ block }: CountdownTimerProps) {
  ...
  return (
    <div className={`flex flex-col items-center gap-8 text-center ${containerClass}`}>
      {cfg.title && (
        <h2 className="text-2xl font-bold md:text-3xl">{cfg.title}</h2>
      )}
      {cfg.subtitle && (
        <p className={`text-lg ${isDark ? "opacity-80" : "text-muted-foreground"}`}>
          {cfg.subtitle}
        </p>
      )}
          ...
          {[
            ...
            <div key={l} className="flex flex-col items-center">
              ...
              {cfg.show_labels !== false && (
                <span
                  className={`mt-2 text-xs font-medium uppercase tracking-widest ${isDark ? "opacity-70" : "text-muted-foreground"}`}
                >
                  {l}
                </span>
              )}
      ...
      {cfg.cta_label && cfg.cta_url && (
        <Link
          ...
          className={`rounded-lg px-8 py-3 font-semibold transition-opacity hover:opacity-90 ${
            isDark ? "bg-white text-gray-950" : "bg-primary text-primary-foreground"
          }`}
        >
          {cfg.cta_label}
        </Link>
