...
export function TwoColumnsBlock({ block }: Props) {
  ...
  const leftClass =
    ratio === "60-40" ? "md:col-span-3" : ratio === "40-60" ? "md:col-span-2" : "";
  const rightClass =
    ratio === "60-40" ? "md:col-span-2" : ratio === "40-60" ? "md:col-span-3" : "";
  const gridCols =
    ratio === "50-50" ? "md:grid-cols-2" : "md:grid-cols-5";
    ...
    if (layout.startsWith("image") && leftImageRelation?.metadata?.url) {
      ...
      return (
        <div className="relative aspect-video overflow-hidden rounded-2xl">
          <Image src={url} alt={leftImageRelation.metadata?.alt as string ?? ""} fill className="object-cover" />
        </div>
      );
    }
    return (
  ...
  const renderRight = () => {
    if (layout.endsWith("image") && rightImageRelation?.metadata?.url) {
      ...
      return (
        <div className="relative aspect-video overflow-hidden rounded-2xl">
          <Image src={url} alt={rightImageRelation.metadata?.alt as string ?? ""} fill className="object-cover" />
        </div>
      );
    }
    return (
