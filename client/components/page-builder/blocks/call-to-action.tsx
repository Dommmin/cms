  ...
  return (
    <div className={`relative rounded-2xl px-8 py-16 ${containerStyle}`}>
      {style === 'image' && bgImageUrl && (
        <Image src={bgImageUrl} alt="" fill className="object-cover opacity-30" />
      )}
      <div className={`relative flex flex-col gap-6 ${alignClass}`}>
        ...
        {cfg.subtitle && (
          <p className={`max-w-2xl text-lg ${isLight ? 'text-muted-foreground' : 'text-white/80'}`}>
            {cfg.subtitle}
          </p>
        )}
            ...
            {cfg.primary_label && cfg.primary_url && (
              <Link
                ...
                className={`rounded-lg px-8 py-3 font-semibold transition-opacity hover:opacity-90 ${
                  isLight ? 'bg-primary text-primary-foreground' : 'bg-white text-gray-950'
                }`}
              >
                {cfg.primary_label}
              </Link>
