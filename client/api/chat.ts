...
export async function getConversation(token: string): Promise<SupportConversation> {
  const { data } = await api.get<SupportConversation>(`/support/conversations/${token}`);
  return data;
}
